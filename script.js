...
function atualizarTexto(novoIndex) {
  aboutText.style.opacity = 0;
  setTimeout(() => {
    index = (novoIndex + textos.length) % textos.length;
    aboutText.textContent = textos[index];
...
aboutText.addEventListener("click", (e) => {
  const metade = aboutText.offsetWidth / 2;
  if (e.offsetX > metade) {
    atualizarTexto(index + 1);
  } else {
    atualizarTexto(index - 1);
  }
});

// Setas do teclado e Enter
...
function configurarInteracao() {
  ...
  div.removeEventListener("click", cliqueHandler);

  if (isMobile) {

    hint.textContent = "clique para continuar";
    div.addEventListener("click", cliqueHandler);
  } else {

    hint.textContent = hint.dataset.original || "pressione ENTER para continuar";
    document.addEventListener("keydown", tecladoHandler);
  }
}

function tecladoHandler(event) {
  if (event.key === "Enter" || event.key === "ArrowRight") {
    mostrarTexto();
...
window.addEventListener("load", configurarInteracao);
window.addEventListener("resize", configurarInteracao);
